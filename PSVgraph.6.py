    ...
    for i in range((read_len - k)+ 1):
        ...
        for key in PSV.keys():
            ...
            if kmer in PSV[key]:
                ...
                print("out ender",ender)
                ender = ender +1 

    if first_match:
        un_read = un_read+1            
    x = MG.number_of_edges()
    if int(x) - prev_edge >20:
...
for node in MG.nodes():
   # node_list[node]=[str(MG.in_degree(node)),str(MG.out_degree(node))]
    if MG.degree(node) == 0:
        rem.append(node)
    else:
        node_list[node]=[str(MG.in_degree(node)),str(MG.out_degree(node))]
...
with open('edge_list.txt','w') as e:
    for key in edge_list.keys():
        if len(edge_list[key])>0:
            e.write("\t".join([str(key),"\t".join(set(edge_list[key])) ]   )+"\n")

with open('node_list.txt','w') as n:
    for key in node_list.keys():
        if len(node_list[key])>0:
            n.write("\t".join([str(key),",".join( node_list[key])  ]  )+"\n")


#sys.stdout.write(str(edge_list)+"\n"+str(node_list))
