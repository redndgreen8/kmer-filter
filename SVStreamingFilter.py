...
print("res has " + str(len(jfRefRes)))
if jfRes[-1] == '':
...
if any([len(jfRes) != exp_result_len,               #make sure that there are the correct number of JF results
        len(jfRefRes) != exp_result_len,            #make sure that there are the correct number of HG JF results
        len(queries) % 2 != 0,                      #make sure there are 2 queries for each variant                
        ]):
    print("one of these is not like the other " + str(len(jfRes)) + " "  + str(exp_result_len) + "  " + str(len(jfRefRes)) + " " + str(len(queries))        )
    sys.exit(1)
...
with open(outfile,'w') as f:
    for idx in range(int(len(queries)/2)):
        ...
        #Output 6 tab-separated columns
        #1- ref sequence
        #2- alt sequence
        ...
        out_string = '{}\t{}\t{}\t{}\t{}\t{}\n'.format(ref_seq,alt_seq,ref_hg_str,ref_il_str,alt_hg_str,alt_il_str)
        f.write(out_string)
