...
print("ref has " + str(len(jfRefRes)))
#if jfRes[-1] == '':
...
#
#
if any([ len(jfRefRes) != exp_result_len,            #make sure that there are the correct number of HG JF results
        len(queries) % 2 != 0,                      #make sure there are 2 queries for each variant
        ]):
    print("one of these is not like the other "   + str(exp_result_len) + "  " + str(len(jfRefRes)) + " " + str(len(queries))        )
    sys.exit(1)
...
with open(outfile,'w') as f:
    ...
    #chr1    16576202        .       A       T       275,9
        ...
        out_string = '{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(contig,pos,ref_seq,alt_seq,ref_hg_str,alt_hg_str,DP)
        f.write(out_string)
