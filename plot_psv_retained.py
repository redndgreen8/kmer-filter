...
if(args.filtered is not None):
    ...
    for line in f:
        ...
        alt=line[16].split(";")[0].split("=")[1]
        alt_il.append( alt)
        if(args.retained is not None):
            for liner in retain:
                liner = liner.split()
              # print( liner[0] , curpos,trueposf[0],int(line[1]),trueposf[0]+ int(liner[0]) )
                if trueposf[0]+ int(liner[0]) == int(line[1])+1:
...
if(args.filtered is not None):
    ...
    cov4p, = plt.plot(truepospass, fp+sp+tp+fop, 'o', color="brown", markeredgewidth=0.0, markersize=1.5)
    ...
    for i in range(len(truepospass)):
        ...
        if round_alt>mean*1.25 or round_alt<min(4,0.25*mean):
            fnt=3
        else:
            fnt=7.5
...
cov4, = plt.plot(trueposf, ff+sf+tf+fof, 'o', color="brown", markeredgewidth=0.0, markersize=1.5, label="total coverage")
